...
def preprocess_barts_label(label):
    # 合并类别1、2、4为前景（1）
...
def preprocess_label(label):
    # 合并类别1、2、4为前景（1）
    merged = np.zeros_like(label)
    merged[(label == 1) | (label == 2) | (label == 4)] = 1
    return merged.astype(np.uint8)
...
class t2Dataset(Dataset):
    ...
    def load_data(self, img_pth, gt_pth):
        ...
        img = img[:, :, ::-1].copy()  # change to BGR
        img -= self.img_mean
        #gt = preprocess_label(gt)
        gt=preprocess_barts_label(gt)

        #img = img.astype(np.float32)
        return img, gt

class t1Dataset_aug(Dataset):
    def __init__(self,data_pth,gt_pth,img_mean,transform=None,PL_Tag=False,aug_transform=True):
